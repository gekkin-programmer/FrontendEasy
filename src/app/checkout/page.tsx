...
function CheckoutContent() {
  ...
  const startPolling = (txId: string) => {
    pollRef.current = setInterval(async () => {
      try {
        const data = await api.get<{ status: string }>(`/payments/status/${txId}`);
    ...
    timeoutRef.current = setTimeout(() => {
      ...
      if (step === 'processing') {
        setStep('failed');
        toast.error("Le délai de paiement a expiré. Veuillez réessayer.");
      }
    }, POLL_TIMEOUT_MS);
  };
  ...
  const handlePayment = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = await api.post<{ transactionId: string }>('/payments/initiate', {
        planType: plan,
        ...
        <div className="bg-white dark:bg-zinc-900 text-black dark:text-white border-4 border-black dark:border-white p-10 shadow-[16px_16px_0px_0px_#000] dark:shadow-[16px_16px_0px_0px_#fff]">
          ...
          {step === 'processing' && (
            <div className="py-20 text-center space-y-6">
              ...
              <h2 className="text-2xl font-black uppercase">En attente de confirmation...</h2>
              <p className="text-sm font-bold text-gray-500 leading-relaxed">
                Un prompt PIN a été envoyé sur votre téléphone.<br/>
                Saisissez votre code secret pour confirmer le paiement.
              </p>
          ...
          {step === 'success' && (
            <div className="py-20 text-center space-y-8">
              ...
              <div>
                <h2 className="text-3xl font-black uppercase mb-2">Paiement confirmé !</h2>
                <p className="text-sm font-bold text-gray-500">
                  Votre abonnement <span className="text-[#3C48F5]">{plan}</span> est maintenant actif.
                </p>
              </div>
              <button
          ...
          {step === 'failed' && (
            <div className="py-20 text-center space-y-8">
              <div className="w-24 h-24 bg-red-500 border-4 border-black dark:border-white mx-auto flex items-center justify-center shadow-[8px_8px_0px_0px_#000] dark:shadow-[8px_8px_0px_0px_#fff]">
                <AlertCircle size={48} className="text-white" />
              </div>
              <div>
                <h2 className="text-3xl font-black uppercase mb-2">Paiement échoué</h2>
                <p className="text-sm font-bold text-gray-500">
                  Le paiement a été annulé ou a expiré. Veuillez réessayer.
                </p>
              </div>
              <button
                onClick={() => setStep('form')}
                className="px-8 py-4 bg-[#3C48F5] text-white font-black uppercase text-xs border-2 border-black dark:border-white shadow-[4px_4px_0px_0px_#000]"
              >
                Réessayer
              </button>
            </div>
  ...
  return (
    <button
      ...
      className={`p-4 border-4 transition-all flex flex-col items-center gap-2 ${
        active
          ? `border-black dark:border-white ${color} shadow-none translate-x-1 translate-y-1`
          : 'border-zinc-200 dark:border-zinc-700 bg-white dark:bg-black shadow-[4px_4px_0px_0px_#000] dark:shadow-[4px_4px_0px_0px_#fff] hover:border-black dark:hover:border-white'
      }`}
    >
