...
// --- NEU COMPONENTS ---
const NeuButton = ({ children, onClick, active, className = "", disabled = false }: any) => (
  <button 
    onClick={onClick}
    disabled={disabled}
    className={`
      ...
      ${active 
        ...
        : 'bg-white text-black hover:bg-yellow-300 shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] active:translate-x-[4px] active:translate-y-[4px] active:shadow-none'
      }
...
const NeuInput = (props: any) => (
  <input 
    {...props}
    className="bg-white border-2 border-black p-2 font-bold text-sm placeholder:text-gray-400 focus:outline-none focus:bg-yellow-50 focus:shadow-[4px_4px_0px_0px_#000] transition-all w-full font-mono"
  />
);
...
export default function DashboardPage() {
    ...
    // --- 1. REACT QUERY: FETCH DATA ---
    
    // A. Workspaces List
    const { data: myWorkspaces = [], isLoading: wsListLoading } = useQuery({
        queryKey: ['workspaces'],
        queryFn: () => api.get<any[]>('/workspaces'),
    });

    // B. Current Workspace (including accounts)
    ...
    // C. Posts (Polled every 15s to check scheduled status)
    ...
    const createPostMutation = useMutation({
        ...
        onSuccess: () => {
            ...
            // Don't change tab, just refresh list
        },
    ...
    const handleAddPost = async (
        ...
        date?: Date, 
        mediaIds?: string[], 
        status: 'DRAFT' | 'SCHEDULED' | 'REVIEW' = 'DRAFT',
        selectedAccountIds?: string[]
    ) => {
        // Fallback targets logic
        const targets = selectedAccountIds && selectedAccountIds.length > 0 
            ? selectedAccountIds 
            : (accounts.length > 0 ? [accounts[0].id] : []);

        if (targets.length === 0) { 
            toast.error("ERR_NO_NODES_SELECTED"); 
            return; 
        }
    ...
    if (loading) return (
        <div className="h-screen flex flex-col items-center justify-center bg-[#FDFBF7] text-black">
            ...
            <p className="font-black text-xl uppercase tracking-widest font-mono">SYSTEM_INIT...</p>
        </div>
    );
        ...
        <div className="min-h-screen bg-[#FDFBF7] font-sans text-black relative selection:bg-yellow-300">
            ...
            <div className="lg:hidden sticky top-0 left-0 right-0 h-16 bg-white border-b-2 border-black z-40 flex items-center justify-between px-4">
                <div className="flex items-center gap-2">
                    ...
                    <div className="font-black text-xl tracking-tighter italic">EASYPOST.</div>
                </div>
                ...
                {isSidebarOpen && (
                    <>
                        ...
                        <motion.aside initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} className="fixed inset-y-0 left-0 w-72 bg-white border-r-4 border-black flex flex-col z-50 shadow-[10px_0px_0px_0px_rgba(0,0,0,0.2)]">
                            <div className="p-6 border-b-2 border-black flex justify-between items-center bg-yellow-400"><span className="font-black text-xl uppercase">Menu</span><button onClick={() => setIsSidebarOpen(false)} className="border-2 border-black bg-white hover:bg-red-500 hover:text-white transition-colors p-1"><X/></button></div>
                            <nav className="p-4 space-y-3">{navItems.map(item => (<SidebarItem key={item.id} icon={item.icon} label={item.label} active={activeTab === item.id} onClick={() => { setActiveTab(item.id as TabType); setIsSidebarOpen(false); }} />))}</nav>
                        </motion.aside>
                    </>
                )}
            </AnimatePresence>
            ...
            <main className="relative z-10 flex flex-col min-h-screen">
                ...
                <header className="hidden lg:flex sticky top-0 z-30 h-20 bg-white/95 backdrop-blur-sm border-b-4 border-black items-center justify-between px-8 shadow-sm">
                            ...
                            <AnimatePresence>
                                {isAccountMenuOpen && (
                                    <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full left-0 mt-2 w-64 bg-white border-2 border-black shadow-[8px_8px_0px_0px_#000] z-50 p-2 origin-top">
                                        <div className="space-y-1">{myWorkspaces.map(ws => (<button key={ws.id} onClick={() => { router.push(`/dashboard/${ws.id}`); setIsAccountMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-left hover:bg-yellow-200 border-2 border-transparent hover:border-black transition-all"><div className="w-5 h-5 border border-black overflow-hidden bg-gray-50"><img src={getAvatarUrl(ws.name)} className="w-full h-full object-cover" /></div><span className="flex-1 font-bold truncate">{ws.name}</span>{currentWorkspace?.id === ws.id && <Check size={16} className="text-blue-600 border-2 border-transparent"/>}</button>))}</div>
                                        <div className="h-0.5 bg-black my-2"/><button onClick={() => { setIsCreateModalOpen(true); setIsAccountMenuOpen(false); }} className="w-full flex items-center gap-2 px-3 py-2 text-sm font-bold text-blue-600 hover:bg-blue-50 border-2 border-transparent hover:border-blue-600 transition-all"><Plus size={16}/> New Workspace</button>
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </div>
                    ...
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2"><NeuInput placeholder="SEARCH_DATABASE..." value={searchTerm} onChange={(e: any) => setSearchTerm(e.target.value)} style={{ width: '250px' }} /><div className="bg-black text-white p-2.5 border-2 border-black"><Search size={18} /></div></div>
                        <VoiceAiButton onCommand={handleVoiceCommand} />
                ...
                <div className="flex-1 px-4 md:px-8 pb-32 pt-8">
                    <div className="max-w-[1600px] mx-auto flex gap-8 items-start">
                        ...
                        <div className="hidden lg:block sticky top-32 z-10 self-start">
                            <QuickConnectSidebar accounts={accounts} workspaceId={workspaceId} refreshData={() => queryClient.invalidateQueries({ queryKey: ['workspace', workspaceId] })} />
                        </div>
                        ...
                        <div className="hidden lg:block w-64 sticky top-32 self-start space-y-4">
                            <div className="p-4 bg-yellow-400 border-2 border-black shadow-[4px_4px_0px_0px_#000]"><h3 className="font-black text-lg uppercase tracking-tight">MENU</h3></div>
                            <nav className="space-y-3">{navItems.map((item) => (<button key={item.id} onClick={() => setActiveTab(item.id as TabType)} className={`w-full flex items-center justify-between p-4 border-2 border-black transition-all duration-200 group ${activeTab === item.id ? 'bg-black text-white shadow-[4px_4px_0px_0px_#000] translate-x-[-2px] translate-y-[-2px]' : 'bg-white text-black hover:bg-gray-100 hover:shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)]'}`}><div className="flex items-center gap-3"><item.icon size={20} strokeWidth={activeTab === item.id ? 2.5 : 2} /><span className="font-bold uppercase tracking-wider">{item.label}</span></div>{activeTab === item.id && <ArrowRight size={16} />}</button>))}</nav>
                            <div className="mt-8 p-4 bg-white border-2 border-black border-dashed"><p className="text-xs font-mono text-gray-500 mb-2">SUBSCRIPTION</p><div className="flex justify-between items-end"><span className="text-xl font-black">PRO</span><button onClick={() => setActiveTab('settings')} className="text-xs font-bold underline hover:text-blue-600">MANAGE</button></div></div>
                        </div>
            ...
            <NeuModal title="CREATE_WORKSPACE" isOpen={isCreateModalOpen} onClose={() => setIsCreateModalOpen(false)}>
                <div className="space-y-4"><div><label className="text-xs font-bold uppercase mb-1 block">Workspace Name</label><NeuInput value={newWorkspaceName} onChange={(e: any) => setNewWorkspaceName(e.target.value)} placeholder="E.G. DIGITAL_AGENCY_KENYA" autoFocus /></div><div className="flex justify-end gap-2"><NeuButton onClick={() => setIsCreateModalOpen(false)} className="bg-white hover:bg-gray-100">Cancel</NeuButton><NeuButton onClick={handleCreateWorkspace} className="bg-[#3C48F6] text-white hover:bg-blue-700">Create</NeuButton></div></div>
            </NeuModal>

            {/* Facebook Page Selector Modal */}
            <FacebookPageSelector isOpen={isFbPageSelectorOpen} onClose={() => setIsFbPageSelectorOpen(false)} onRefresh={() => queryClient.invalidateQueries({ queryKey: ['workspace', workspaceId] })} exchangeToken={tempExchangeToken} />
        </div>
    );
}

// --- SUB COMPONENTS (Optimized for React Query) ---
        ...
        <NeuModal title="SELECT_ENTITY" isOpen={isOpen} onClose={onClose}>
            {isLoading ? <div className="p-8 flex justify-center"><Loader2 className="animate-spin" /></div> : (
                <div className="max-h-[300px] overflow-y-auto space-y-2">
                    {pages.length === 0 && <p className="text-center text-sm font-mono">NO_ENTITIES_FOUND</p>}
                    {pages.map((page) => (
                        <button key={page.id} onClick={() => selectMutation.mutate(page)} className="w-full flex items-center gap-3 p-3 border-2 border-black hover:bg-yellow-100 transition-all text-left">
...
const QuickConnectSidebar = ({ accounts, workspaceId, refreshData }: any) => {
    ...
    // Connect needs page redirect, can't be mutation
    ...
    return (
        <div className="w-16 flex flex-col items-center gap-4 py-6 bg-white border-2 border-black shadow-[6px_6px_0px_0px_#000]">
            ...
            {platforms.map((p) => {
                const connectedAccount = accounts.find((a:any) => a.platform?.toLowerCase() === p.id.toLowerCase());
                return (
                    <div key={p.id} className="relative group">
                        {connectedAccount ? (
                            <>
                                <button className="w-10 h-10 flex items-center justify-center border-2 border-black bg-gray-100 text-black opacity-50 cursor-default"><p.Icon size={18} /></button>
                                <button onClick={() => { if(confirm("CONFIRM_TERMINATION?")) disconnectMutation.mutate(connectedAccount.id) }} className="absolute inset-0 w-10 h-10 flex items-center justify-center border-2 border-black bg-red-500 text-white opacity-0 group-hover:opacity-100 transition-opacity z-10 cursor-pointer"><Trash2 size={16} /></button>
                                <div className="absolute -top-1 -right-1 pointer-events-none z-20"><div className="w-4 h-4 bg-green-500 border-2 border-black flex items-center justify-center text-white"><Check size={10} strokeWidth={4} /></div></div>
                            </>
                        ) : (
                            <button onClick={() => handleConnect(p.id)} className="w-10 h-10 flex items-center justify-center border-2 border-black bg-white hover:bg-black hover:text-white cursor-pointer shadow-[2px_2px_0px_0px_#000] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-none transition-all"><p.Icon size={18} /></button>
                        )}
                    </div>
                );
            })}
            <div className="h-0.5 w-8 bg-black my-2"></div>
            <button className="text-gray-400 hover:text-black transition-colors"><ExternalLink size={16} /></button>
        </div>
    );
};
