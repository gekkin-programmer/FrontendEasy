 ...
 */
const _createPostToolSchema = z.object({
  content: z
    .string()
    .describe('The main text content of the social media post.'),
  platformIds: z
    .array(z.string())
    .describe('An array of social account IDs to publish to.'),
  scheduleDate: z
    .string()
    .datetime()
    .optional()
    .describe(
      'ISO string for scheduling date (e.g., "2026-02-01T10:00:00Z"). If omitted, post immediately.',
    ),
  mediaIds: z
    .array(z.string())
    .optional()
    .describe(
      'An array of media IDs (e.g., from your media service) to include in the post.',
    ),
...
const analyticsToolSchema = z.object({
  ...
  workspaceId: z
    .string()
    .optional()
    .describe('The ID of the workspace for which to fetch analytics.'),
  period: z
    .nativeEnum(AnalyticsPeriod)
    .optional()
    .describe('Time period for the analytics (e.g., "MONTH").'),
  startDate: z
    .string()
    .datetime()
    .optional()
    .describe('Start date for custom period (ISO string).'),
  endDate: z
    .string()
    .datetime()
    .optional()
    .describe('End date for custom period (ISO string).'),
  limit: z
    .number()
    .int()
    .positive()
    .optional()
    .describe('Maximum number of items to return (e.g., for top posts).'),
});
        ...
        try {
          ...
          switch (metric) {
            ...
            case 'accounts':
              ...
              return JSON.stringify(
                await analyticsService.getAccountsAnalytics(
                  workspaceId,
                  strictQueryDates,
                ),
              );
            ...
            case 'posts':
              ...
              return JSON.stringify(
                await analyticsService.getTopPosts(
                  workspaceId,
                  strictQueryDates,
                  limit || 10,
                ),
            ...
            case 'best-time':
              return JSON.stringify(
                await analyticsService.analyzeBestTimes(workspaceId),
              );

            case 'hashtags':
              return JSON.stringify(
                await analyticsService.analyzeHashtags(workspaceId),
              );

            case 'content-mix':
              return JSON.stringify(
                await analyticsService.analyzeContentMix(workspaceId),
              );

            case 'health':
              return JSON.stringify(
                await analyticsService.analyzeAccountHealth(workspaceId),
              );

            case 'forecast':
              return JSON.stringify(
                await analyticsService.calculateGrowthForecast(workspaceId),
              );
            ...
            default:
              return 'Unknown analytics metric or metric not implemented.';
          }
        } catch (error) {
