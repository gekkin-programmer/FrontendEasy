...
export class FacebookService implements ISocialPlatform {
  ...
  async getHistory(
    ...
    try {
      ...
      const fields = 'id,message,created_time,full_picture,attachments{media,media_type,subattachments},shares,comments.summary(true){id,message,created_time,from},likes.summary(true),insights.metric(post_impressions_unique)';
