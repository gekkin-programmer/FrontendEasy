...
export class EngagementService {
  ...
  async findAll(workspaceId: string) {
    ...
    return comments.map((c) => ({
      ...
      authorHandle:
        c.platform === 'TWITTER' ? `@${c.authorName}` : c.authorName,
  ...
  async reply(commentId: string, text: string, _workspaceId: string) {
    ...
    try {
      if (comment.platform === SocialPlatform.FACEBOOK && comment.externalId) {
        ...
        comment.platform === SocialPlatform.INSTAGRAM &&
        comment.externalId
      ) {
        ...
        comment.platform === SocialPlatform.TWITTER &&
        comment.externalId
      ) {
    ...
    for (const acc of accounts) {
      try {
        ...
        if (acc.platform === SocialPlatform.INSTAGRAM && acc.platformUserId) {
          externalPosts = await this.instagramService.getHistory(
            acc.accessToken,
            acc.platformUserId,
          );
        } else if (
          acc.platform === SocialPlatform.FACEBOOK &&
          acc.platformUserId
        ) {
          externalPosts = await this.facebookService.getHistory(
            acc.accessToken,
            acc.platformUserId,
          );
        } else if (
          ...
          externalPosts = await this.twitterService.getHistory(
            acc.accessToken,
            acc.platformUserId,
          );
        }
            ...
            for (const ec of ep.comments) {
              ...
              await this.prisma.postComment.upsert({
                ...
                create: {
                  ...
                  post: {
                    connect: {
                      id: await this.findPostIdByExternalId(ep.externalId),
                    },
                  },
                },
              });
        ...
        this.logger.warn(
          `Failed to sync ${acc.platform} for ${workspaceId}: ${e.message}`,
        );
      }
    }
  ...
  private async findPostIdByExternalId(
    externalId: string,
