...
describe('MembersService - MVP Tests', () => {
  ...
  describe('invite', () => {
    ...
    it('should invite member via email and send invitation email', async () => {
      ...
      mockPrismaService.workspaceMember.findUnique.mockResolvedValueOnce({ role: 'OWNER' });
      mockPrismaService.workspace.findUnique.mockResolvedValue({ id: workspaceId, name: 'Test Workspace' });
      
      // 2. User exists check mock
      mockPrismaService.user.findUnique.mockResolvedValueOnce(null); // Shadow user logic
      mockPrismaService.user.create.mockResolvedValue({ id: 'user-new', email: dto.email });
      
      // 3. Duplicate check mock
      mockPrismaService.workspaceMember.findUnique.mockResolvedValueOnce(null);
      
      mockPrismaService.workspaceMember.create.mockResolvedValue({ id: 'mem-1' });
      ...
      expect(mockPrismaService.workspaceMember.create).toHaveBeenCalledWith(expect.objectContaining({
        data: expect.objectContaining({ role: dto.role })
      }));
    });

    it('should prevent non-admin from inviting', async () => {
      mockPrismaService.workspaceMember.findUnique.mockResolvedValueOnce({ role: 'VIEWER' });
      await expect(service.invite(workspaceId, dto, inviterId)).rejects.toThrow(ForbiddenException);
    });
  });

  describe('list and update', () => {
    it('should list workspace members', async () => {
      mockPrismaService.workspaceMember.findMany.mockResolvedValue([{ id: 'm1' }, { id: 'm2' }]);
    ...
    it('should update member role (admin only)', async () => {
      ...
      mockPrismaService.workspaceMember.findUnique.mockResolvedValueOnce({ role: 'ADMIN' });
      mockPrismaService.workspaceMember.update.mockResolvedValue({ id: 'm1', role: 'ADMIN' });

      const result = await service.updateRole('ws-1', 'm1', { role: 'ADMIN' as any }, 'admin-id');
