...
describe('PublishingService - MVP Tests', () => {
  ...
  describe('publishPost', () => {
    ...
    it('should publish post to Facebook and track results', async () => {
      ...
      expect(mockPrismaService.postSocialAccount.update).toHaveBeenCalledWith(expect.objectContaining({
        where: { id: 'psa1' },
        data: expect.objectContaining({ status: 'PUBLISHED', platformPostId: 'fb-post-123' })
      }));
      ...
      const multiPost = {
        ...
        socialAccounts: [
          ...mockPost.socialAccounts,
          {
            id: 'psa2',
            status: 'SCHEDULED',
            socialAccount: { id: 'sa2', platform: 'TWITTER', accessToken: 'tw-token' },
          }
        ]
      };
      mockPrismaService.post.findUnique.mockResolvedValue(multiPost);
      mockedAxios.post.mockResolvedValue({ data: { id: 'plat-id', data: { id: 'tw-id' } } }); // Mock both FB/TW formats

      await service.publishPost(postId);

      expect(mockedAxios.post).toHaveBeenCalledTimes(2);
      expect(mockPrismaService.postSocialAccount.update).toHaveBeenCalledTimes(2);
    });
    ...
    it('should handle partial failures (1 success, 1 fail)', async () => {
        const multiPost = {
          ...mockPost,
          socialAccounts: [
            { id: 'psa1', status: 'SCHEDULED', socialAccount: { id: 'sa1', platform: 'FACEBOOK', accessToken: 't1' } },
            { id: 'psa2', status: 'SCHEDULED', socialAccount: { id: 'sa2', platform: 'TWITTER', accessToken: 't2' } }
          ]
        };
        mockPrismaService.post.findUnique.mockResolvedValue(multiPost);
        ...
        expect(mockPrismaService.postSocialAccount.update).toHaveBeenCalledWith(expect.objectContaining({
            where: { id: 'psa2' },
            data: expect.objectContaining({ status: 'FAILED', errorMessage: 'Twitter down' })
        }));
    ...
    it('should prevent double publication if post already PUBLISHED', async () => {
        mockPrismaService.post.findUnique.mockResolvedValue({ ...mockPost, status: 'PUBLISHED' });
    ...
    it('should handle rate limiting (generic error handling)', async () => {
        ...
        expect(mockPrismaService.postSocialAccount.update).toHaveBeenCalledWith(expect.objectContaining({
            data: expect.objectContaining({ status: 'FAILED', errorMessage: 'Rate limit reached' })
        }));
    });
  });
});
