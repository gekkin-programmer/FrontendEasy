...
describe('SchedulingService - MVP Tests', () => {
  ...
  describe('schedulePost', () => {
    it('should schedule post for future date', async () => {
      ...
      mockPrismaService.post.findUnique.mockResolvedValue({ id: 'p1', status: 'DRAFT' });
      mockPrismaService.post.update.mockResolvedValue({ id: 'p1', status: 'SCHEDULED', scheduledFor: futureDate });
    ...
    it('should reject past date', async () => {
      ...
      await expect(service.schedulePost('p1', pastDate)).rejects.toThrow('Cannot schedule post in the past');
    });

    it('should prevent scheduling same post twice', async () => {
      const futureDate = new Date(Date.now() + 100000);
      mockPrismaService.post.findUnique.mockResolvedValue({ id: 'p1', status: 'SCHEDULED' });
      
      await expect(service.schedulePost('p1', futureDate)).rejects.toThrow('Post is already scheduled or published');
    });
  });
  ...
  describe('reschedule and cancel', () => {
    it('should reschedule post to new time', async () => {
      ...
      mockPrismaService.post.update.mockResolvedValue({ id: 'p1', scheduledFor: newDate });
    ...
    it('should cancel scheduled post', async () => {
      mockPrismaService.post.update.mockResolvedValue({ id: 'p1', status: 'DRAFT' });
  ...
  describe('listing', () => {
    it('should list scheduled posts', async () => {
      mockPrismaService.post.findMany.mockResolvedValue([{ id: 'p1', status: 'SCHEDULED' }]);
  ...
  describe('automation', () => {
    it('should auto-publish post at scheduled time', async () => {
      const now = new Date();
      mockPrismaService.post.findMany.mockResolvedValue([{ id: 'p1', status: 'SCHEDULED' }]);
      mockPrismaService.post.update.mockResolvedValue({ id: 'p1', status: 'PUBLISHING' });
