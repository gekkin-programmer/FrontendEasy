...
export class AnalyticsService {
  ...
  async getAnalytics(workspaceId: string, filters: AnalyticsFilterDto) {
    const { period = AnalyticsPeriod.MONTH, type = AnalyticsType.OVERVIEW } =
      filters;
  ...
  public async getOverviewAnalytics(
    ...
    const [totalPosts, engagementStats, scheduledPosts, draftsCount] =
      await Promise.all([
        ...
        this.prisma.post.count({
          where: { workspaceId, status: PostStatus.DRAFT },
        }),
      ]);
  ...
  public async getAccountsAnalytics(
    workspaceId: string,
    dateRange: { startDate: Date; endDate: Date },
  ) {
  ...
  public async analyzeBestTimes(workspaceId: string) {
    ...
    const data = await this.smartSchedulingService.getHeatmap(
      workspaceId,
      'FACEBOOK',
    );
  ...
  public async analyzeContentMix(workspaceId: string) {
    ...
    posts.forEach((post) => {
      ...
      const mediaCount = Array.isArray(post.mediaUrls)
        ? (post.mediaUrls as any[]).length
        : 0;
      let type = 'TEXT';
    ...
    return Object.entries(stats).map(([type, data]) => ({
      ...
      count: data.count,
      avgEngagement:
        data.count > 0 ? Math.round(data.totalEng / data.count) : 0,
    }));
  }
  ...
  public async analyzeAccountHealth(workspaceId: string) {
    ...
    if (posts.length < 2)
      return { healthScore: 0, consistency: 'New 🐣', streak: 0 };
    ...
    const variance =
      gaps.reduce((acc, val) => acc + Math.pow(val - avgGap, 2), 0) /
      (gaps.length || 1);
  ...
  public async calculateGrowthForecast(workspaceId: string) {
    ...
    if (posts.length < 5)
      return { trend: 'Not enough data', nextMonthEstimate: 0 };
    ...
    const trendDirection =
      slope > 0 ? 'Growing 🚀' : slope < 0 ? 'Declining 📉' : 'Stagnant ➡️';

    return {
      trend: trendDirection,
      currentAvgEngagement: Math.round(sumY / n),
      forecastNextMonth: Math.round(
        predictedEngagement > 0 ? predictedEngagement : 0,
      ),
  ...
  public async analyzeSmartCopy(workspaceId: string) {
    ...
    const totalEng = posts.reduce((acc, p) => {
      let eng = 0;
      p.socialAccounts.forEach(
        (sa) => (eng += (sa.likes || 0) + (sa.comments || 0)),
      );
      return acc + eng;
    }, 0);
    ...
    switch (period) {
      ...
      case AnalyticsPeriod.CUSTOM:
        if (!customStartDate || !customEndDate)
          throw new BadRequestException('Dates required');
