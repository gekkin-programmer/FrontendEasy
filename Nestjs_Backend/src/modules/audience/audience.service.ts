...
export class AudienceService {
  ...
  async trackInteraction(workspaceId: string, data: {
  ...
  private async updateMemberLoyalty(memberId: string) {
    ...
    const engagementScore = member.interactions.reduce((sum, i) => sum + i.weight, 0);

    // B. Loyalty Score Factors
    // 1. Consistency: (posts engaged with / total posts)
    const uniquePostsEngaged = new Set(member.interactions.map(i => i.postId).filter(id => !!id)).size;
    ...
    const recentInteractions = member.interactions.filter(i => i.createdAt >= thirtyDaysAgo).length;
    const frequency = (recentInteractions / 4) * 10; // Normalized

    // 3. Tenure: Days since first interaction
    const firstInteraction = member.interactions.reduce((min, i) => i.createdAt < min ? i.createdAt : min, new Date());
    const tenureDays = Math.floor((new Date().getTime() - firstInteraction.getTime()) / (1000 * 60 * 60 * 24));
    const tenureScore = Math.min(tenureDays, 100);

    // Final Loyalty Score (Average of factors)
    const loyaltyScore = (consistency * 0.5) + (frequency * 0.3) + (tenureScore * 0.2);
  ...
  async syncAudience(workspaceId: string) {
    ...
    const mockData = [
      { platformUserId: 'f1', username: 'SuperFan_99', displayName: 'John Doe', type: 'LIKE' },
      { platformUserId: 'f1', username: 'SuperFan_99', displayName: 'John Doe', type: 'COMMENT' },
      { platformUserId: 'f2', username: 'TechGuru', displayName: 'Alex Smith', type: 'SHARE' },
      { platformUserId: 'f3', username: 'Lurker_01', displayName: 'Sarah W.', type: 'LIKE' },
    ];
