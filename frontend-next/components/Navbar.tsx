...
export default function Navbar() {
  ...
  return (
    /* 
      Fix 1: Changed position to absolute, top-0, left-0 to overlay perfectly onto your background light. 
      Fix 2: Switched 'grid-cols-3' to 'flex justify-between' for flawless responsive window scaling.
    */
    <nav className="absolute top-0 left-0 w-full h-[87px] flex items-center justify-between px-[40px] md:px-[74px] text-white z-50 box-border">
      ...
      */}
          ...
          {activeDropdown === 'Canaux' && (
            <div className="absolute left-0 top-[87px] w-full bg-[#040028] cursor-default p-[40px] shadow-2xl font-sans">
              <div className="max-w-[1440px] mx-auto grid grid-cols-4 gap-x-[40px] gap-y-[32px]">
                ...
                <div className="text-white/80 hover:text-white transition-colors cursor-pointer group/item flex items-center gap-[16px]">
                  <svg width="40" height="40" viewBox="0 0 32 32" fill="none" xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0">
                    <circle cx="16" cy="16" r="14" fill="url(#canva_grad)"/>
                    <path d="M12 18L16 12L20 18" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                    <defs>
                      <linearGradient id="canva_grad" x1="0" y1="0" x2="32" y2="32" gradientUnits="userSpaceOnUse">
      ...
      <div className="flex items-center justify-end gap-[16px] md:gap-[24px] font-['Rubik_One'] font-normal">
        ...
        <div className="relative">
          ...
          {isLangOpen && (
            <div className="absolute right-0 top-[40px] w-[240px] bg-white border border-[#E5E5E5] rounded-[8px] shadow-[0px_12px_16px_-4px_rgba(0,0,0,0.08),0px_4px_6px_-2px_rgba(0,0,0,0.03)] py-[8px] flex flex-col z-50">
              <div 
                className="flex flex-row items-center px-[16px] h-[40px] cursor-pointer hover:bg-gray-50 transition-colors"
                onClick={() => { setLanguage('en'); setIsLangOpen(false); }}
              >
                <span className="font-['Inter'] font-medium text-[14px] text-[#404040]">English</span>
              </div>
              <div 
                className="flex flex-row items-center px-[16px] h-[40px] cursor-pointer hover:bg-gray-50 transition-colors"
                onClick={() => { setLanguage('fr'); setIsLangOpen(false); }}
              >
                <span className="font-['Inter'] font-medium text-[14px] text-[#404040]">Français</span>
              </div>
              <div 
                className="flex flex-row items-center px-[16px] h-[40px] cursor-pointer hover:bg-gray-50 transition-colors"
                onClick={() => { setLanguage('ar'); setIsLangOpen(false); }}
              >
                <span className="font-['Inter'] font-medium text-[14px] text-[#404040]">العربية (Arab)</span>
              </div>
            </div>
          )}
        </div>
