...
function StrategyView({ workspaceId }: { workspaceId: string }) {
    ...
    // 1. Account Health (FIXED TYPE ERROR)
    ...
    // 2. Growth Forecast (FIXED TYPE ERROR)
    ...
    // 4. Content Mix (FIXED TYPE ERROR)
    ...
    // 5. Smart Copy & Hashtags (FIXED TYPE ERROR)
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="h-full overflow-y-auto pr-2 pb-20 scrollbar-hide">
            ...
            {/* ROW 1: HEALTH & FORECAST (The "Investor" Metrics) */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-6">
                
                {/* 1. Account Health */}
                <NeuCard title="Account_Health" icon={Activity} className="bg-yellow-50">
                    <div className="flex items-end justify-between mb-4">
                        ...
                        <span className="bg-black text-white px-3 py-1 font-bold uppercase text-xs mb-2">{health.consistencyStatus}</span>
                    </div>
                    <div className="w-full bg-gray-200 h-4 border-2 border-black">
                        <div className="bg-green-500 h-full transition-all duration-1000" style={{ width: `${health.healthScore}%` }}></div>
                    </div>
            ...
            {/* ROW 2: INTELLIGENCE (Timing & Copy) */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                
                {/* 4. Golden Windows (Best Time) */}
                ...
                <NeuCard title="Power_Words" icon={Zap} className="lg:col-span-1">
                    <div className="flex flex-wrap gap-2 content-start h-full">
                        {smartCopyQuery.data?.map((item: any, i: number) => (
                            <span 
                                key={i} 
                                className="px-2 py-1 border-2 border-black bg-white font-bold text-xs uppercase hover:bg-yellow-300 transition-colors cursor-default"
...
// ============================================================================
// VIEW 2: LIVE MONITOR (Posts List - Unchanged but included for completeness)
// ============================================================================
function LiveStreamView({ workspaceId }: { workspaceId: string }) {
    ...
    const syncMutation = useMutation({
        mutationFn: async () => {
            ...
            if (accounts.length > 0) {
                await api.post(`/social-accounts/${accounts[0].id}/sync`, {});
            }
        },
        onSuccess: () => {
            setTimeout(() => {
    ...
    const filteredPosts = posts.filter((p: AnalyticsPost) => 
    ...
    return (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex flex-col md:flex-row gap-8 h-full pb-20">
            ...
            <div className="w-full md:w-[380px] flex flex-col bg-white border-2 border-black shadow-[8px_8px_0px_0px_#000] flex-shrink-0 h-full">
                <div className="p-4 border-b-2 border-black bg-yellow-400">
                    <div className="flex items-center justify-between mb-4">
                        <h3 className="font-black text-lg uppercase flex items-center gap-2"><TrendingUp className="w-5 h-5 text-black" strokeWidth={3} /> Live_Stream</h3>
                        <button 
                            onClick={() => syncMutation.mutate()} 
                            disabled={syncMutation.isPending}
                            className="bg-black text-white px-2 py-1 text-[10px] font-bold uppercase hover:bg-gray-800 disabled:opacity-50 flex items-center gap-1 transition-all"
                        >
                    ...
                    <div className="relative">
                        ...
                        <input type="text" placeholder="SEARCH_POSTS..." className="w-full pl-9 pr-4 py-2 bg-white border-2 border-black text-sm font-bold placeholder:text-gray-400 focus:outline-none focus:bg-yellow-50 focus:shadow-[2px_2px_0px_0px_#000] transition-all uppercase" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
                    </div>
                </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-white border-2 border-black shadow-[8px_8px_0px_0px_#000] h-full overflow-hidden">
                {selectedPostId ? (
                    <PostAnalyticsDetailWrapper postId={selectedPostId} />
                ) : (
...
function PostAnalyticsDetailWrapper({ postId }: { postId: string }) {
    // 🟢 FETCH SINGLE POST (Robust Error Handling)
    const { data: post, isLoading, error } = useQuery({
        queryKey: ['post-analytics', postId],
        queryFn: async () => {
            try {
                const res: any = await api.get(`/posts/${postId}`);
                return res.data || res;
            } catch (err) {
    ...
    if (error || !post) return (
        <div className="flex-1 flex flex-col items-center justify-center p-10 text-center">
            ...
            <div className="font-bold text-lg">POST DATA UNAVAILABLE</div>
            <p className="text-sm text-gray-500 max-w-xs mt-2">This post might have been deleted or the connection to the platform was lost.</p>
        </div>
    );
...
function PostListCard({ post, engagement, isSelected, onClick }: { post: AnalyticsPost, engagement: number, isSelected: boolean, onClick: () => void }) {
    ...
    return (
        <div onClick={onClick} className={cn("p-4 cursor-pointer transition-all duration-150 relative border-b-2 border-black group", isSelected ? "bg-black text-white" : "bg-white hover:bg-yellow-50 text-black")}>
            ...
            <p className={cn("text-sm font-bold line-clamp-2 mb-3 leading-snug", isSelected ? "text-gray-200 pl-3" : "text-black")}>
...
function PostAnalyticsDetail({ post }: { post: AnalyticsPost }) {
    ...
    if (hasReachData) {
        ...
        engagementRate = "-"; // Cannot calculate safely
        reachDisplay = "No Data"; 
    }

    // 3. Real Chart Data (Interaction Breakdown)
    const chartData = [
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-0 divide-x-2 divide-black border-b-2 border-black bg-white">
                ...
                <div className="p-6 flex flex-col justify-center items-center text-center bg-yellow-50">
                    ...
                    {!hasReachData && metrics.likes > 0 && (
                        <span className="text-[10px] text-gray-500 mt-1 font-mono">WAITING FOR REACH DATA</span>
                    )}
                </div>
            </div>

            {/* Real Data Chart: Interaction Breakdown */}
                        ...
                        <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={chartData} layout="vertical" margin={{ left: 20, right: 20, top: 20, bottom: 20 }}>
                                ...
                                <Tooltip 
                                    ...
                                    contentStyle={{ backgroundColor: '#fff', borderRadius: '0px', border: '2px solid #000', boxShadow: '4px 4px 0px 0px #000', color: '#000', fontWeight: 'bold', textTransform: 'uppercase' }} 
                                />
